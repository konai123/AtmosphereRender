...
float valueNoise3D(const in float3 x)
{
...
float worleyNoise(const in float3 p, const in float cellCount)
{
	...
	for (int xo = -1; xo <= 1; xo++)
	{
		for (int yo = -1; yo <= 1; yo++)
		{
			for (int zo = -1; zo <= 1; zo++)
			{
				...
				tp = pCell - tp - valueNoise3D(mod(tp, cellCount / 1.0));
				d = min(d, dot(tp, tp));
			}
		}
	}
...
float noiseInterpolation(const in float2 i_coord, const in float i_size)
{
	...
	float p0 = Hash2D(randomInput);
	float p1 = Hash2D(randomInput + float2(1.0, 0.0));
	float p2 = Hash2D(randomInput + float2(0.0, 1.0));
	float p3 = Hash2D(randomInput + float2(1.0, 1.0));
...
float weatherNoise(const in float2 p, const in float scale, in float frequency, in float aplitude, in uint octaves)
{
	float noiseValue = 0.0;
	float localAplitude = aplitude;
	float localFrecuency = frequency;

	for (int index = 0; index < octaves; index++)
	{

		noiseValue += noiseInterpolation(p, scale * localFrecuency) * localAplitude;
...
float perlinNoise(const in float3 p, in float frequency, in int octaves)
{
	...
	{
		const float3 pIn = p * float3(frequency, frequency, frequency);
		float v = pnoise(float4(pIn, 0.0), float4(frequency, frequency, frequency, frequency));
...
float pnoise(float4 P, float4 rep)
{
	...
	float4 Pi1 = mod((Pi0 + float4(1,1,1,1)), rep); // Integer part + 1 mod rep
	Pi0 = mod289(Pi0);
	Pi1 = mod289(Pi1);
